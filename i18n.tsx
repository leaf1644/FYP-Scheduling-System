...
const dictionaries: Record<Locale, Record<string, string>> = {
  en: {
    ...
    'errors.noStudentsFound': 'No students were parsed from the uploaded file.',
    'errors.noRoomsFound': 'No usable room data was parsed from the uploaded file.',
    'errors.noRoomSlotsFound': 'Room data was parsed, but no usable room-slot combinations were generated.',
    ...
    'errors.emptyScheduleResult': 'The solver returned an empty schedule result. Please check the uploaded files and try again.',
    'errors.incompleteScheduleResult': 'The solver returned an incomplete schedule result. Please check the uploaded files and try again.',
    'errors.malformedScheduleResult': 'The solver returned malformed schedule data.',
    ...
    'uploads.preferences.label': '5. Professor Preference File (Optional)',
    'uploads.preferences.description': 'Optional. Use a separate CSV/XLSX with professorId or professorName plus preferenceType, weight, and optional target.',
    'form.solver': 'Solver',
    ...
    'solver.ilpWarning': 'PuLP ILP can become much slower when the schedule contains many interacting hard and soft constraints. In those cases, the system may skip the second soft-constraint optimization phase and keep only the best coverage result.',
    'solver.ilpRecommendation': 'Recommendation: use CP-SAT when you have many professor preferences or tightly constrained schedules.',
    ...
    'dashboard.prefBreakdown': 'Professor Preference Status',
    'dashboard.prefBreakdownHint': 'Shows only professors whose preferences were changed from the default setting, and whether each one is currently satisfied.',
    'dashboard.prefSatisfied': 'Satisfied',
    'dashboard.prefUnsatisfied': 'Not satisfied',
    'dashboard.prefPenalty': 'Penalty: {penalty}',
    'dashboard.prefWeight': 'Weight {weight}',
    'dashboard.prefLoad': 'Assigned load: {count}',
    'dashboard.prefSummary.concentrate': 'Current schedule uses {usedDays} day(s).',
    'dashboard.prefSummary.maxPerDay': 'Current maximum is {maxPerDay} per day, target is {target}.',
    'dashboard.prefSummary.spread': 'Current schedule uses {usedDays} day(s), ideal target is {idealDays}.',
    ...
    'dashboard.empty.title': 'No scheduled presentations to display',
    'dashboard.empty.withUnscheduled': 'The solver could not place any students into valid room slots. Review the unscheduled list below or switch to manual adjustment mode.',
    'dashboard.empty.noData': 'No schedule data is available to display. Please re-upload the files and run the scheduler again.',
  ...
  'zh-Hans': {
    ...
    'errors.noStudentsFound': '上传的学生文件中没有解析出任何学生资料。',
    'errors.noRoomsFound': '上传的房间文件中没有解析出可用的房间资料。',
    'errors.noRoomSlotsFound': '房间资料已解析，但没有生成任何可用的房间时段组合。',
    'errors.validationFailed': '数据验证失败，请检查文件内容与 ID 对应。',
    'errors.scheduleFailed': '排程计算失败。',
    'errors.parseFailed': '文件解析失败，请确认 CSV/XLSX 格式。',
    'errors.emptyScheduleResult': '求解器返回了空的排程结果，请检查上传文件后重试。',
    'errors.incompleteScheduleResult': '求解器返回了不完整的排程结果，请检查上传文件后重试。',
    'errors.malformedScheduleResult': '求解器返回的排程资料格式不正确。',
    ...
    'uploads.preferences.label': '5. 教授偏好文件（可选）',
    'uploads.preferences.description': '可选。使用独立 CSV/XLSX，包含 professorId 或 professorName，以及 preferenceType、weight 与可选的 target。',
    'form.solver': '求解器',
    'solver.cpSat': 'CP-SAT（Python）',
    'solver.pulp': 'PuLP ILP（Python / CBC）',
    'solver.legacy': 'Legacy Python Heuristic',
    'solver.help': '三个选项都会调用你新增的 Python solver。只有在本地 API 不可用时，系统才会退回内建 worker。',
    'solver.ilpWarning': 'PuLP ILP 在存在多种 hard constraint 与 soft constraint 相互作用时会明显更慢。在这种情况下，系统可能会跳过第二阶段软约束优化，只保留最佳覆盖率结果。',
    'solver.ilpRecommendation': '建议：当教授偏好较多或限制较紧时，优先使用 CP-SAT。',
    ...
    'dashboard.prefBreakdown': '教授偏好状态',
    'dashboard.prefBreakdownHint': '只显示相对于默认设定有改动的教授，并标示这些偏好在最终排程下是否已满足。',
    'dashboard.prefSatisfied': '已满足',
    'dashboard.prefUnsatisfied': '未满足',
    'dashboard.prefPenalty': '罚分：{penalty}',
    'dashboard.prefWeight': '权重 {weight}',
    'dashboard.prefLoad': '已分配负载：{count}',
    'dashboard.prefSummary.concentrate': '目前排程使用了 {usedDays} 天。',
    'dashboard.prefSummary.maxPerDay': '目前单日最高为 {maxPerDay} 场，目标为 {target}。',
    'dashboard.prefSummary.spread': '目前排程使用了 {usedDays} 天，理想目标为 {idealDays} 天。',
    ...
    'dashboard.empty.title': '目前没有可显示的已排程答辩',
    'dashboard.empty.withUnscheduled': '求解器未能将任何学生安排进有效房间时段。请查看下方未排程原因，或切换到手动调整模式。',
    'dashboard.empty.noData': '目前没有可显示的排程资料，请重新上传文件后再试。',
  ...
  'zh-Hant': {
    ...
    'errors.noStudentsFound': '上傳的學生檔案中沒有解析出任何學生資料。',
    'errors.noRoomsFound': '上傳的房間檔案中沒有解析出可用的房間資料。',
    'errors.noRoomSlotsFound': '房間資料已解析，但沒有產生任何可用的房間時段組合。',
    'errors.validationFailed': '資料驗證失敗，請檢查檔案內容與 ID 對應。',
    'errors.scheduleFailed': '排程計算失敗。',
    'errors.parseFailed': '檔案解析失敗，請確認 CSV/XLSX 格式。',
    'errors.emptyScheduleResult': '求解器回傳了空的排程結果，請檢查上傳檔案後再試。',
    'errors.incompleteScheduleResult': '求解器回傳了不完整的排程結果，請檢查上傳檔案後再試。',
    'errors.malformedScheduleResult': '求解器回傳的排程資料格式不正確。',
    ...
    'uploads.preferences.label': '5. 教授偏好檔（可選）',
    'uploads.preferences.description': '可選。使用獨立 CSV/XLSX，包含 professorId 或 professorName，以及 preferenceType、weight 與可選的 target。',
    'form.solver': '求解器',
    'solver.cpSat': 'CP-SAT（Python）',
    'solver.pulp': 'PuLP ILP（Python / CBC）',
    'solver.legacy': 'Legacy Python Heuristic',
    'solver.help': '三個選項都會呼叫你新增的 Python solver。若本地 API 無法使用，系統才會退回內建 worker。',
    'solver.ilpWarning': 'PuLP ILP 在存在多種 hard constraint 與 soft constraint 互相作用時會明顯更慢。在這種情況下，系統可能會跳過第二階段軟限制優化，只保留最佳覆蓋率結果。',
    'solver.ilpRecommendation': '建議：當教授偏好較多或限制較緊時，優先使用 CP-SAT。',
    ...
    'dashboard.prefBreakdown': '教授偏好狀態',
    'dashboard.prefBreakdownHint': '只顯示相對於預設設定有改動的教授，並標示這些偏好在最終排程下是否已滿足。',
    'dashboard.prefSatisfied': '已滿足',
    'dashboard.prefUnsatisfied': '未滿足',
    'dashboard.prefPenalty': '罰分：{penalty}',
    'dashboard.prefWeight': '權重 {weight}',
    'dashboard.prefLoad': '已分配負載：{count}',
    'dashboard.prefSummary.concentrate': '目前排程使用了 {usedDays} 天。',
    'dashboard.prefSummary.maxPerDay': '目前單日最高為 {maxPerDay} 場，目標為 {target}。',
    'dashboard.prefSummary.spread': '目前排程使用了 {usedDays} 天，理想目標為 {idealDays} 天。',
    ...
    'dashboard.csv.scheduled': '已排程',
    'dashboard.csv.unscheduled': '未排程',
    'dashboard.empty.title': '目前沒有可顯示的已排程口試',
    'dashboard.empty.withUnscheduled': '求解器未能將任何學生安排進有效房間時段。請查看下方未排程原因，或切換到手動調整模式。',
    'dashboard.empty.noData': '目前沒有可顯示的排程資料，請重新上傳檔案後再試。',
...
export const localizeValidationIssue = (message: string, t: TranslateFn): string => {
  if (message === '沒有可排程的學生資料。') {
    return t('errors.noStudentsFound');
  }

  if (message === '沒有可用的房間資料。') {
    return t('errors.noRoomsFound');
  }

  if (message === '沒有可用的時段資料。') {
    return t('errors.noSlotsFound');
  }
