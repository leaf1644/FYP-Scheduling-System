import { RoomSlot, ScheduleAssignment, ScheduleResult, Student } from '../types';
...
export interface LegacySolverPayload {
  students: Student[];
...
export async function solveLegacySchedule(payload: LegacySolverPayload): Promise<ScheduleResult> {
  const { students, allRoomSlots, profAvailability, profPreferences, timeoutMs } = payload;
